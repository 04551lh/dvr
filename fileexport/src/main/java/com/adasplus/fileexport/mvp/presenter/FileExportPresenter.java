...
import com.adasplus.fileexport.adapter.ChannelsNumberAdapter;
import com.adasplus.fileexport.adapter.StreamTypeAdapter;
import com.adasplus.fileexport.mvp.contract.IFileExportContract;
import com.adasplus.fileexport.mvp.contract.OnItemChannelNumbersClickListener;
import com.adasplus.fileexport.mvp.contract.OnItemStreamTypeClickListener;
import com.adasplus.fileexport.mvp.model.ChannelNumbersModel;
...
public class FileExportPresenter implements IFileExportContract.Presenter, View.OnClickListener, OnItemChannelNumbersClickListener, OnItemStreamTypeClickListener {
    ...
    private FileExportActivity mFileExportActivity;
    ...
    public void initData() {
        mTvFileType = mFileExportView.getTvFileType();
        mTvSelectDate = mFileExportView.getTvSelectDate();
        mTvStartTime = mFileExportView.getTvStartTime();
        mTvEndTime = mFileExportView.getTvEndTime();
        mTvChannel = mFileExportView.getTvChannel();
        mTvStreamType = mFileExportView.getTvStreamType();
        mPbExportProgressbar = mFileExportView.getPbExportProgressbar();
        mTvCurrentExportSize = mFileExportView.getTvCurrentExportSize();
        mTvChannelValue = mFileExportView.getTvChannelValue();
        mTvStreamTypeValue = mFileExportView.getTvStreamTypeValue();

        mChannelsNumberAdapter = new ChannelsNumberAdapter();
        mStreamTypeAdapter = new StreamTypeAdapter();
        mWidth = (int) mFileExportActivity.getResources().getDimension(R.dimen.dp_200);

        isSelectLog(true);
        String all = mFileExportActivity.getResources().getString(R.string.all);
        String main_stream = mFileExportActivity.getResources().getString(R.string.main_stream);
        String child_stream = mFileExportActivity.getResources().getString(R.string.child_stream);
        ...
        mChannels = mFileExportActivity.getResources().getString(R.string.channels);
        ...
        BaseWrapper.getInstance().getFileExport().subscribe(new Subscriber<FileExportModel>() {
            ...
            public void onError(Throwable e) {
                ExceptionUtils.exceptionHandling(mFileExportActivity, e);
            }
    ...
    public void initListener() {
        ...
        tvExportFile.setOnClickListener(this);
    ...
    private void selectStreamType() {
        View view = View.inflate(mFileExportActivity, R.layout.popup_common, null);
        RecyclerView rv_list = view.findViewById(R.id.rv_list);

        rv_list.setLayoutManager(new LinearLayoutManager(mFileExportActivity, RecyclerView.VERTICAL, false));
        mStreamTypeAdapter.setData(mStreamTypeList);
        rv_list.setAdapter(mStreamTypeAdapter);

        mStreamTypePopupWindow = new CommonPopupWindow.Builder(mFileExportActivity)
                .setView(view)
    ...
    private void selectChannelNumbers() {
        ...
        mChannelNumbersPopupWindow = new CommonPopupWindow.Builder(mFileExportActivity)
                .setView(view)
                .setWidthAndHeight(mWidth, LinearLayout.LayoutParams.WRAP_CONTENT)
                .setOutsideTouchable(true)
                .create();
    ...
    private void selectTime(final String type) {
        TimePickerView timePickerView = new TimePickerBuilder(mFileExportActivity, new OnTimeSelectListener() {
            @Override
    ...
    private void selectDate() {
        String select_date = mFileExportActivity.getResources().getString(R.string.select_date);
        Calendar date = Calendar.getInstance();
        TimePickerView pickerView = new TimePickerBuilder(mFileExportActivity, new OnTimeSelectListener() {
    ...
    private void showFileTypePopup() {
        View view = View.inflate(mFileExportActivity, R.layout.popup_file_type, null);
        ...
        mFileTypePopupWindow = new CommonPopupWindow.Builder(mFileExportActivity)
                .setView(view)
                .setWidthAndHeight(mWidth, LinearLayout.LayoutParams.WRAP_CONTENT)
                .create();
