...
public class SpeedSetPresenter implements ISpeedSetContract.Presenter, View.OnClickListener, SlideSwitchView.OnSwitchStatusChangeListener {
    ...
    public void initData() {
        ...
        SettingsWrapper.getInstance().getDefaultBySpeedSet().subscribe(new Subscriber<SpeedSetModel>() {
            @Override
    ...
    private void saveSpeedSetData() {
        ...
        //保存最新的设置速度的最新配置
        SettingsWrapper.getInstance().updateSpeedSet(jsonObject).subscribe(new Subscriber<SpeedSetModel>() {
            @Override
            public void onCompleted() {
