...
public class ParamsSetPresenter implements IParamsSetContract.Presenter, View.OnClickListener {
    ...
    public void initData() {
        ...
        //获取部标设备中的参数填写的数据
        SettingsWrapper.getInstance().getParamsSet().subscribe(new Subscriber<ParamsSetModel>() {
            @Override
        ...
        if (id == R.id.iv_back) {
            ...
            if (mParamsSetModel != null) {
                ...
                try {
                    ...
                    //更改参数填写
                    SettingsWrapper.getInstance().updateParamsSet(jsonObject).subscribe(new Subscriber<ParamsSetModel>() {
                        @Override
                        public void onCompleted() {
