...
public class DMSWarningPresenter implements IDMSWarningContract.Presenter, View.OnClickListener, SlideSwitchView.OnSwitchStatusChangeListener, ISwitchItemClickListener {
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.iv_back) { //点击返回按钮的监听
            ...
            //恢复报警的默认设置
            SettingsWrapper.getInstance().dmsRestoreDefaultSettings().subscribe(new Subscriber<WarningsRestoreDefaultSettingsModel>() {
                @Override
            ...
            if (mDmsWarningModel != null){
                ...
                try {
                    ...
                    SettingsWrapper.getInstance().updateDMSSet(jsonObject).subscribe(new Subscriber<DMSWarningModel>() {
                        @Override
                        public void onCompleted() {
    ...
    private void getDMSDefaultSet() {
        SettingsWrapper.getInstance().getDMSSet().subscribe(new Subscriber<DMSWarningModel>() {
            @Override
            public void onCompleted() {
