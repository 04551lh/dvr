...
import com.adasplus.settings.adapter.DeviceFormatAdapter;
import com.adasplus.settings.mvp.contract.IDeviceFormatContract;
import com.adasplus.settings.mvp.contract.IDeviceFormatItemListener;
...
public class DeviceFormatPresenter implements IDeviceFormatContract.Presenter, View.OnClickListener, IDeviceFormatItemListener {
    ...
    public void initData() {
        ...
        //获取存储器的列表
        SettingsWrapper.getInstance().getStorageInfoList().subscribe(new Subscriber<DeviceFormatModel>() {
            @Override
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.iv_back) {
            ...
            if (mArray != null && mArray.size() > 0) {
                ...
                //开始格式化磁盘的接口请求
                SettingsWrapper.getInstance().updateStorageFormatList(storageDeviceArray).subscribe(new Subscriber<DeviceFormatModel>() {
                    @Override
                    public void onCompleted() {
